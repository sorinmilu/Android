...
- [Script Live Coding - Simple Hello World cu Buton Quit](#script-live-coding---simple-hello-world-cu-buton-quit)
    ...
    - [Pași Live Coding](#pa%C8%99i-live-coding)
        ...
        - [Pasul 8: Crearea clasei MainActivity.java](#pasul-8-crearea-clasei-mainactivityjava)
        - [Pasul 9: Verificarea structurii proiectului](#pasul-9-verificarea-structurii-proiectului)
...
**Actiune** Creez structura completă de directoare pentru aplicația Android, inclusiv directorul pentru layout-uri XML.
...
**Actiune** Creez fișierul `settings.gradle` la rădăcina proiectului pentru configurarea modulelor Gradle.
...
**Actiune** Creez fișierul `build.gradle` la nivelul rădăcină care configurează plugin-ul Android pentru Gradle.
...
**Actiune** Creez fișierul `gradle.properties` pentru configurații globale Gradle.
...
**Actiune** Creez fișierul `build.gradle` pentru modulul `app` care definește configurația aplicației Android.
...
**Note:** "Configurez tipurile de build și opțiunile de compilare Java la versiunea 11."
...
**Note:** "În blocul `dependencies` adaug două biblioteci: `appcompat` pentru `AppCompatActivity` și `constraintlayout` pentru `ConstraintLayout`. Această ultimă dependență este nouă față de aplicația Hello World - este necesară pentru a folosi `ConstraintLayout` în fișierul XML de layout."
...
**Actiune** Creez fișierul `AndroidManifest.xml` care declară componentele aplicației.
...
**Note:** "Creez fișierul `AndroidManifest.xml` în `app/src/main/`. Declar tag-ul `manifest` cu namespace-urile necesare. În tag-ul `application` setez label-ul aplicației la 'Alkrilki01' și tema la `Theme.AppCompat.Light.NoActionBar` pentru un fundal clar fără bara de acțiuni."
...
**Note:** "Declar activitatea `MainActivity` cu `android:exported="true"` pentru a permite sistemului să o lanseze. `intent-filter` cu acțiunea `MAIN` și categoria `LAUNCHER` face ca această activitate să fie punctul de intrare al aplicației."
...
**Actiune** Creez fișierul XML de layout care definește interfața utilizatorului. Aceasta este diferența principală față de aplicația Hello World anterioară.
...
**Note:** "Setez ID-ul layout-ului la `@+id/main` - simbolul `@+id/` creează un nou ID în clasa R generată automat. `match_parent` pentru lățime și înălțime face ca layout-ul să ocupe tot ecranul. `tools:context` specifică activitatea asociată, folosită de instrumentele de dezvoltare."
...
**Note:** "Adaug un `TextView` pentru textul 'Hello World'. `layout_width="match_parent"` face ca textul să ocupe toată lățimea ecranului. `wrap_content` pentru înălțime înseamnă că se ajustează la conținut. Setez dimensiunea textului la 42sp și activez majusculele cu `textAllCaps`."
...
**Note:** "Adaug un `Button` cu ID-ul `quitButton` - acest ID va fi folosit în Java pentru a obține referința la buton. Lățimea este `match_parent` pentru a ocupa toată lățimea, înălțimea este fixă la 87dp. Adaug un margin de 20dp în partea de jos pentru spațiere."
...
**Note:** "Setez constrângerile pentru buton: `layout_constraintBottom_toBottomOf="parent"` fixează butonul în partea de jos a ecranului, iar `layout_constraintEnd_toEndOf` aliniază la dreapta. ConstraintLayout va calcula automat poziționarea corectă."
...
**Note:** "Închid tag-ul `ConstraintLayout`. Layout-ul este complet - avem un TextView centrat sus și un buton fixat jos, cu constrângeri care leagă elementele între ele."
...
**Actiune** Creez clasa Java principală care folosește layout-ul XML și gestionează evenimentele.
...
**Note:** "Creez fișierul `MainActivity.java` în directorul `app/src/main/java/ro/makore/akrilki_01/`. Declar pachetul care corespunde cu structura de directoare."
...
**Note:** "Adaug import-urile necesare: `Bundle` pentru starea activității, `AppCompatActivity` ca clasă de bază, și `Button` pentru gestionarea butonului. Observați că nu mai importăm `LinearLayout` sau `TextView` - acestea sunt definite în XML, nu în Java."
...
**Note:** "Definesc clasa `MainActivity` care extinde `AppCompatActivity` și suprascriu metoda `onCreate`. Apelez `super.onCreate` pentru inițializarea corectă."
...
**Note:** "Folosesc metoda `findViewById()` pentru a obține referința la butonul definit în XML. `R.id.quitButton` corespunde cu `android:id="@+id/quitButton"` din fișierul XML. Aceasta este pattern-ul standard în Android pentru conectarea codului Java cu elementele UI definite în XML."
...
**Note:** "Atașez un listener de evenimente la buton folosind o expresie lambda. Când butonul este apăsat, se execută `finishAffinity()` care închide toate activitățile din aplicație și iese complet. Lambda-ul `v ->` primește view-ul care a declanșat evenimentul, dar nu îl folosim în acest caz."
...
**Note:** "Închid metoda `onCreate` și clasa. Aplicația este completă - interfața este definită în XML, iar logica de gestionare a evenimentelor este în Java."
...
**Actiune** Verific că toate fișierele necesare sunt prezente și că structura este corectă.
...
**Actiune** Compilez aplicația pentru a genera APK-ul de debug.
...
**Note:** "Navighez în directorul proiectului și rulez `gradle build` pentru a compila aplicația. Această comandă va genera APK-ul de debug în `app/build/outputs/apk/debug/app-debug.apk`. Gradle va procesa și fișierul XML de layout, generând clasele R necesare."
...
**Actiune** Verific ce dispozitive Android sunt conectate și disponibile pentru instalare.
...
**Actiune** Instalez APK-ul de debug pe dispozitivul conectat.
...
**Actiune** Lansez activitatea principală a aplicației pe dispozitiv.
...
**Note:** "Folosesc `adb shell am start` pentru a lansa activitatea. Opțiunea `-n` specifică componenta prin pachet și numele clasei. Aplicația se va deschide pe dispozitiv și va afișa 'Hello World' sus și butonul 'Quit' jos, conform layout-ului XML definit."
...
**Actiune** Monitorizez logurile aplicației pentru a verifica comportamentul.
